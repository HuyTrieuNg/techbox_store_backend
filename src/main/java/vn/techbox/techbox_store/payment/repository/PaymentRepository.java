...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    @Query("SELECT SUM(opi.finalAmount) FROM Payment opi WHERE opi.paymentStatus = 'PAID' AND opi.paymentCompletedAt BETWEEN :startDate AND :endDate")
    BigDecimal getTotalRevenueByDateRange(@Param("startDate") LocalDateTime startDate, @Param("endDate") LocalDateTime endDate);

    @Query("SELECT COUNT(opi) FROM Payment opi WHERE opi.paymentMethod = :method AND opi.paymentStatus = 'PAID'")
